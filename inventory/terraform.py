...
def aws_host(resource, tfvars=None):
    name = resource['primary']['attributes'].get('tags.Name', '')
    raw_attrs = resource['primary']['attributes']
    ...
    attrs = {
        ...
        'ebs_optimized': parse_bool(raw_attrs.get('ebs_optimized', 'f')),
        ...
        # NEEDS FIX: as we are passing group id instead of name
        # https://www.terraform.io/docs/providers/aws/r/security_group.html#security_groups
...
def main():
    ...
    # Default to environment variable if exitsclear
    default_root = os.environ.get('TFSTATE_ROOT') or os.path.abspath(os.path.join(os.path.dirname(__file__),
                                                '..', '..', ))
